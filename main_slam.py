...
def draw_associated_cameras(viewer3D, assoc_est_poses, assoc_gt_poses, T_gt_est):
    ...
    R_est_gt = (
        T_est_gt[:3, :3] / scale
    )  # we need a pure rotation to avoid camera scale changes
    assoc_gt_poses_aligned = [np.eye(4) for i in range(len(assoc_gt_poses))]
    for i in range(len(assoc_gt_poses)):
        assoc_gt_poses_aligned[i][:3, 3] = (
            T_est_gt[:3, :3] @ assoc_gt_poses[i][:3, 3] + T_est_gt[:3, 3]
        )
...
if __name__ == "__main__":
    ...
    if config.trajectory_saving_settings["save_trajectory"]:
        trajectory_online_file_path, trajectory_final_file_path = (
    ...
    feature_tracker_config = FeatureTrackerConfigs.SUPERPOINT  # .ORB2
    ...
    if Parameters.kUseDepthEstimatorInFrontEnd:
        ...
        Printer.green(
            f"Depth_estimator_type: {depth_estimator_type.name}, max_depth: {max_depth}"
        )
    ...
    if groundtruth is not None:
        ...
        viewer3D.set_gt_trajectory(
            gt_traj3d, gt_timestamps, align_with_scale=is_monocular
        )
            ...
            if img is not None:
                ...
                if img is not None:
                    ...
                    if depth is None and depth_estimator is not None:
                        ...
                        depth_img = img_from_depth(
                            depth_prediction, img_min=0, img_max=50
                        )
                        cv2.imshow("depth prediction", depth_img)

                    slam.track(
                        img, img_right, depth, img_id, timestamp
                    )  # main SLAM function
