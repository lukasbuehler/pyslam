...
KeyNetAffNetHardNetFeature2D = import_from(
    "feature_keynet_affnet_hardnet", "KeyNetAffNetHardNetFeature2D"
)
...
class FeatureManager:
    def __init__(
        ...
        self.sigma_level0 = (
            sigma_level0  # sigma of the keypoint localization at level 0
        )
        ...
        if self.descriptor_type in [
            ...
            Printer.orange(
                "FeatureManager: Forcing scale factor=2 for detector",
        ...
        if (
            ...
            self._feature_detector = SuperPointFeature2D(
                do_cuda=False
            )  # Set the cuda to the actual setting
            ...
            if self.descriptor_type != FeatureDescriptorTypes.NONE:
                ...
                self.pyramid_type = PyramidType.GAUSS_PYRAMID
                self.pyramid_do_parallel = (
                    False  # N.B.: XFEAT interface class is not thread-safe!
                )
            ...
            self.num_levels = 1  # force
            self.need_color_image = True
            self._feature_detector = LightGlueSIFTFeature2D(
                num_features=self.num_features
            )
            self.keypoint_filter_type = KeyPointFilterTypes.NONE
            #
            ...
            # self.num_levels = - # internally recomputed
            self._feature_detector = KeyNetAffNetHardNetFeature2D(
                num_features=self.num_features
            )
            self.keypoint_filter_type = KeyPointFilterTypes.NONE
            #
        ...
        if self.is_detector_equal_to_descriptor:
            ...
            if (
                ...
                self._feature_descriptor = (
                    self._feature_detector
                )  # reuse the same detector object
                #
                #
        ...
        try:
            self.max_descriptor_distance = FeatureInfo.max_descriptor_distance[
    ...
    def set_num_features(self, num_features):
        ...
        if self._feature_detector is not None:
            try:
                ...
                Printer.orange(
                    "WARNING: could not set nFeatures in detector: ",
                    self.detector_type.name,
                )
        if self._feature_descriptor is not None:
            try:
                ...
                Printer.orange(
                    "WARNING: could not set nFeatures in descriptor: ",
                    self.detector_type.name,
                )
